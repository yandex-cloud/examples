...
locals {
  ...
  k8s_version           = "" # Set the version of Kubernetes.
...
resource "yandex_vpc_default_security_group" "default-sg" {
  description = "Group rules for Yandex Managed Service for Gitlab and Yandex Managed Service for Kubernetes cluster. Apply it to the cluster and node groups."
  network_id  = yandex_vpc_network.k8s-network.id

  ingress {
    description    = "For work with the Git repository over SSH protocol."
    protocol       = "TCP"
  ...
  ingress {
    description    = "For connect to Yandex Container Registry."
    protocol       = "TCP"
    v4_cidr_blocks = ["0.0.0.0/0"]
    port           = 5050
  }
