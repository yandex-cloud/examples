...
locals {
  ...
  target_pgsql_version = "" # Set the PostgreSQL version. It must be the same or higher than the version in the source cluster.
...
variable "pg-extensions" {
  description = "Required extensions for the Managed Service for PsotgreSQL cluster"
  type        = set(string)
  default = [
