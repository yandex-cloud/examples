...
function publishToDevice(deviceDataService, iotCoreDeviceId) {
 ...
 const powerVoltageSensorValue = (parseFloat(process.env.POWER_SENSOR_VALUE) +
Math.random()).toFixed(2);
 const temperatureSensorValue = (parseFloat(process.env.TEMPERATURE_SENSOR_VALUE)
+ Math.random()).toFixed(2);
 ...
 "TimeStamp":"${timeStamp}",
 "Values":[
 {"Type":"Bool","Name":"Service door
sensor","Value":"${serviceDoorSensorValue}"},
 {"Type":"Float","Name":"Power
Voltage","Value":"${powerVoltageSensorValue}"},
{"Type":"Float","Name":"Temperature","Value":"${temperatureSensorValue}"},
 {"Type":"Float","Name":"Cash drawer
fullness","Value":"${cashDrawerSensorValue}"},
 {"Items":[
 {"Type":"Float", "Id":"1","Name":"Item
1","Fullness":"${item1SensorValue}"},
 {"Type":"Float", "Id":"2","Name":"Item
2","Fullness":"${item2SensorValue}"},
 {"Type":"Float", "Id":"3","Name":"Item
3","Fullness":"${item3SensorValue}"},
 {"Type":"Float", "Id":"4","Name":"Item
4","Fullness":"${item4SensorValue}"}
 ]}
 ]
 }`
 ),
...
async function publishToAllDevicesInRegistry(session, registryId) {
 ...
 console.log(`found ${devices.devices.length} devices in registry
${registryId}`);
 await Promise.all(
