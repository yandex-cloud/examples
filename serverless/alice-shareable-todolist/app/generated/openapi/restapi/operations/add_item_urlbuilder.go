...
func (o *AddItemURL) Build() (*url.URL, error) {
	...
	var _path = "/api/todo-lists/{list_id}/items"

	listID := o.ListID
	if listID != "" {
		_path = strings.Replace(_path, "{list_id}", listID, -1)
	} else {
		return nil, errors.New("listId is required on AddItemURL")
	}
...
func (o *AddItemURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on AddItemURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on AddItemURL")
	}
